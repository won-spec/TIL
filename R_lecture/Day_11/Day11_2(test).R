...
df1 <- filter(df,displ <= 4);head(df1)
df2 <- filter(df,displ >= 5);head(df2)
mean(df1$hwy)
mean(df2$hwy)
cat("배기량 4이하 : ",mean(df1$hwy),"배기량 5이상 : ",mean(df2$hwy))
